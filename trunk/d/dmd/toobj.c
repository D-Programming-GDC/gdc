...
void Module::genmoduleinfo()
{
    ...
    if (needmoduleinfo)
	dti32(&dt, 0, true);		// flags (4 means MIstandalone)
    else
	dti32(&dt, 4, true);		// flags (4 means MIstandalone)
...
#else
    sdtor = d_gcc_aggregate_dtors(this);
#endif
...
void VarDeclaration::toObjFile()
{
	...
	{   s->Sdt = init->toDt();
		...
		{   integer_t value;
			...
			databits.resize(dim);
			databits.set();
			dtnbits(& s->Sdt, databits.allocdim * sizeof(databits.data[0]), (char *)databits.data, sizeof(databits.data[0]));
		    } else {
