...
IRState::emitLocalVar(VarDeclaration * v, bool no_init)
{
    ...
    tree var_decl = v->toSymbol()->Stree;
    tree init_exp = NULL_TREE;

    assert( ! TREE_STATIC(var_decl ));
	...
	if (! v->init->isVoidInitializer()) {
	    ...
	    do {
		if (ie->op == TOKassign) {
		    AssignExp * a_exp = (AssignExp *) ie;
		    if (a_exp->e1->op == TOKvar &&
			((VarExp *) a_exp->e1)->var == v) {

			DECL_INITIAL( var_decl ) = convertForAssignment(a_exp->e2, v->type);
			break;
		    }
		}
    ...
    // %% Do we want to do this for special variables?
    if (inVolatile() && DECL_INITIAL( var_decl ) != NULL_TREE) {
	TREE_THIS_VOLATILE( DECL_INITIAL( var_decl )) = 1;
    }
#if D_GCC_VER < 40
    ...
    } else if ( DECL_INITIAL( var_decl )) {
	g.ofile->doLineNote(v->loc);
	expand_decl_init(var_decl); // %%new: merge with above, irs->expandDecl
    } else {
	d_warning(OPT_Wuninitialized, "uninitialized variable '%s'", v->ident ? v->ident->string : "(no name)");
    }
#else
    // pushdecl will allow the VAR_DECL to be placed in a BIND_EXPR when
    // endBindings is called, but we want the initialization *here*
    if (no_init) {
	// nothing
    } else if ( init_exp ) {
	//g.ofile->doLineNote(v->loc);
	...
	init_exp = DECL_INITIAL( var_decl );
	DECL_INITIAL( var_decl ) = NULL_TREE;
	init_exp = build2(MODIFY_EXPR, void_type_node, var_decl, init_exp);
	//g.ofile->doLineNote(v->loc);
	//expand_decl_init(var_decl); // %%new: merge with above, irs->expandDecl
    } else {
	d_warning(OPT_Wuninitialized, "uninitialized variable '%s'", v->ident ? v->ident->string : "(no name)");
...
IRState::convertTo(tree exp, Type * exp_type, Type * target_type)
{
    ...
    if (exp_type->equals(target_type))
	return exp;
	...
	if (target_type->ty == Tclass) {
		...
		if (offset == OFFSET_RUNTIME) {
		    ...
		    return build(COND_EXPR, t,
			boolOp(NE_EXPR, exp, nullPointer()),
			build(PLUS_EXPR, t, exp, size_int(offset)),
			nullPointer());
		} else {
    ...
    case Tsarray:
	{
		...
		d_uns64 sz_b = target_type->next->size();
	...
	if (exp_type->isreal() && target_type->isimaginary() ||
	    ...
	    result = build(COMPLEX_EXPR, target_type->toCtype(), c1, c2);
	} else {
...
IRState::convertForAssignment(Expression * exp, Type * target_type)
{
	...
	if (count) {
	    CtorEltMaker ce;
	    ce.cons( build(RANGE_EXPR, Type::tsize_t->toCtype(),
...
IRState::convertForCondition(tree exp_tree, Type * exp_type) {
	...
	if (TYPE_MODE(TREE_TYPE(a)) == TYPE_MODE(TREE_TYPE(b)))
	    ...
	    result = build(TRUTH_OR_EXPR, TREE_TYPE(a), a, b);
	}
	break;
    case Tdelegate:
...
IRState::boundsCond(tree index, tree upper_bound, bool inclusive)
{
    ...
    bound_check = build(inclusive ? LE_EXPR : LT_EXPR, boolean_type_node,
...
{
    assert(msg->type == Type::tchar->arrayOf());
...
static const char * libcall_ids[LIBCALL_count] =
    { "_d_assert", "_d_assert_msg", "_d_array_bounds", "_d_switch_error",
      ...
      "_d_delclass", "_d_delarray", "_d_delmemory", "_d_callfinalizer",
...
tree IRState::maybeExpandSpecialCall(tree call_exp)
{
	...
	if (DECL_BUILT_IN_CLASS(callee) == NOT_BUILT_IN) {
		...
		*/
		...
		t = TREE_VALUE(TREE_OPERAND(call_exp, 1));
		...
		*/
		...
		{
		    tree val_arg = TREE_VALUE(TREE_OPERAND(call_exp, 1));
		    ...
		    t = TREE_VALUE(TREE_CHAIN(TREE_OPERAND(call_exp, 1)));
		    if ( TREE_CODE( t ) == ADDR_EXPR ) {
			t = TREE_OPERAND(t, 0);
		    }		
...
IRState::attributes(Expressions * in_attrs)
{
	...
	else if (e->op == TOKcall)
	{
		...
		for (unsigned ai = 0; ai < c->arguments->dim; ai++) {
		    // g.irs?
		    args.cons( ((Expression *) c->arguments->data[ai])->toElem(&gen) );
		}
	    }
	}
	else
...
/* Return the static chain to be used when invoking the nested function F.
   F may also be a type with a function context and indicates the
   static chain is for some function nested directly in that context.  This
   allowed so that it is not necessary to find a method of a nested class. */
tree IRState::getStaticChain(tree t)
{
#if D_GCC_VER < 40
    rtx chain_rtx = NULL_RTX;
#else
    tree context_f = NULL_TREE;
#endif
	...
	Search upwards through the nested function chain until a function
	at the same level as the methods X of is found.  The result may,
	in fact, be a method of X.  If the current function is F, just
	use virtual_stack_vars_rtx.

	*/
	
	do  {
	    t = get_containing_scope(t);
	} while (t && TREE_CODE( t ) != FUNCTION_DECL );

	/* Make sure we can access the frame. */

	if (t != current_function_decl) {
	    tree f = current_function_decl;
	    tree c = f;

	    do {
		f = c;
		if (DECL_NO_STATIC_CHAIN(f))
		    goto cannot_access_frame;
		c = decl_function_context(f);
	    } while (c && c != t);
...
#if D_GCC_VER < 40
	    chain_rtx = virtual_stack_vars_rtx;
#else
	    context_f = t;
#endif
	}
...
#if D_GCC_VER < 40
    if (! chain_rtx)
	chain_rtx = lookup_static_chain(t);

    tree result = make_node (RTL_EXPR);
    TREE_TYPE (result) = ptr_type_node;
    RTL_EXPR_RTL (result) = chain_rtx;
    return result;
#else
    if (! context_f)
	context_f = decl_function_context(t); // duplicates last decl_function_context call
    gcc_assert(context_f != NULL_TREE && TREE_CODE(context_f) == FUNCTION_DECL);
    return build1(STATIC_CHAIN_EXPR, ptr_type_node, context_f);
#endif
}
...
IRState::toElemLvalue(Expression * e)
{
    ...
    */
	...
	if (array_type->ty == Taarray) {
	    ...
	    args[2] = this->integerConstant( array_type->next->size(), Type::tsize_t );
