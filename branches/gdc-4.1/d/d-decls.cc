...
Symbol *VarDeclaration::toSymbol()
{
	...
	if (storage_class & STCparameter)
	    decl_kind = PARM_DECL;
	else if (isConst() && ! gen.isDeclarationReferenceType( this ) &&
	    type->isscalar() &&
	    /* i386 asm needs VAR_DECL for loading floating point constants.
	       That should be fixed instead of this exception. */
	    ! type->isfloating() 
		 )
	...
	if (nestedref && decl_kind != CONST_DECL) {
...
Symbol *FuncDeclaration::toSymbol()
{
	...
	if (! isym) {
	    ...
	    } else if (isMain() && func_type->next->toBasetype()->ty == Tvoid) {
...
Symbol *Module::toSymbol()
{
	...
	*/
	...
	csym->Stree = decl;
	csym->ScontextDecl = g.ofile->fileContext;
