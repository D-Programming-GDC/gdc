...
public:
  ...
  void visit(AndAndExp *e)
  {
	...
	this->result_ = build_condition(build_ctype(e->type), t1, t2, void_node);
      }
  }
  ...
  void visit(OrOrExp *e)
  {
	...
	this->result_ = build_condition(build_ctype(e->type), cond, t2, void_node);
      }
  }
  ...
  void visit(CatAssignExp *e)
  {
	...
	if ((tb2->ty == Tarray || tb2->ty == Tsarray)
	...
	else if (d_types_same(etype, tb2))
	  {
	    ...
	    this->result_ = maybe_compound_expr(expr, result);
	  }
	else
	  gcc_unreachable();
      }
  ...
  void visit(AssertExp *e)
  {
	...
	tree cond = build_boolop(NE_EXPR, arg, null_pointer_node);
	this->result_ = build_vcondition(cond, invc ? invc : void_node, assert_call);
      }
    else
      {
  ...
  void visit(ArrayLiteralExp *e)
  {
	...
	if (!initializer_zerop(value))
	  {
	    ...
	    if (init != NULL_TREE)
	      saved_elems = maybe_vcompound_expr(saved_elems, init);
	...
	this->result_ = maybe_compound_expr(saved_elems, d_convert(type, ctor));
      }
    else
      {
	tree args[2];
	...
	this->result_ = maybe_compound_expr(saved_elems, result);
      }
  }
  ...
  void visit(StructLiteralExp *e)
  {
	...
	this->result_ = maybe_compound_expr(saved_elems, ctor);
	return;
      }
	...
	init = maybe_compound_expr(init, saved_elems);
...
build_expr_dtor(Expression *e)
{
  ...
  if (dtors != NULL_TREE)
	  ...
	  && ((DotVarExp *) ce->e1)->var->isCtorDeclaration())
	{
	  ...
	  expr = maybe_vcompound_expr(expr, result);
...
build_return_dtor(Expression *e, Type *type, TypeFunction *tf)
{
  ...
  result = maybe_vcompound_expr(expr, return_expr(result));
