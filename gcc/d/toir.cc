...
add_stmt (tree t)
{
  /* Ignore (void) 0; expression statements received from the frontend.
     Likewise void_zero_node is used when contracts become nops in
     release code.  */
...
public:
  ...
  void visit (IfStatement *s)
  {
    ...
    tree ifcond = convert_for_condition (build_expr_dtor (s->condition),
					 s->condition->type);
    tree ifbody = void_zero_node;
    tree elsebody = void_zero_node;
  ...
  void visit (DoStatement *s)
  {
    ...
    tree exitcond = convert_for_condition (build_expr_dtor (s->condition),
					   s->condition->type);
    add_stmt (build_vcondition (exitcond, void_zero_node,
	...
	for (size_t i = 0; i < s->cases->dim; i++)
	  {
		...
		tree ifbody = fold_build1 (GOTO_EXPR, void_type_node,
					   caselabel);
		tree cond = build_vcondition (ifcase, ifbody, void_zero_node);
