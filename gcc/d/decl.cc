...
make_alias_for_thunk (tree function)
{
  ...
  if (!flag_syntax_only)
    {
      cgraph_node *aliasn;
      aliasn = cgraph_same_body_alias (NULL, alias, function);
...
finish_thunk (tree thunk, tree function)
{
  ...
  if (TARGET_USE_LOCAL_THUNK_ALIAS_P (function)
    ...
    {
      tree fn = function;
      struct symtab_node *symbol = symtab_get_node (function);
      ...
      if (DECL_SECTION_NAME (fn) != NULL && DECL_ONE_ONLY (fn))
	{
	  ...
	  DECL_SECTION_NAME (thunk) = DECL_SECTION_NAME (fn);
	}
    }
  ...
  funcn = cgraph_get_create_node (function);
  gcc_assert (funcn);
  thunk_node = cgraph_add_thunk (funcn, thunk, thunk,
  ...
  if (!targetm.asm_out.can_output_mi_thunk (thunk, fixed_offset,
					    0, alias))
    {
      /* if varargs... */
...
get_vtable_decl (ClassDeclaration *decl)
{
  ...
  /* For GCC <= 4.9 use this to avoid checking asserts in ctor_for_folding
     in varasm.c */
