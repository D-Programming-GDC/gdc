...
unsigned setMangleOverride(Dsymbol *s, char *sym);
void setIdentifier(UnitTestDeclaration *utd);
bool canInferAttributes(FuncDeclaration *fd, Scope *sc);
void initInferAttributes(FuncDeclaration *fd);
...
public:
    ...
    void visit(PragmaDeclaration *pd)
    {
        ...
        else if (pd->ident == Id::lib)
        {
            ...
            else
            {
                ...
                StringExp *se = e->toStringExp();
                if (!se)
                    pd->error("string expected for library name, not '%s'", e->toChars());
                else
                {
        ...
        else if (pd->ident == Id::mangle)
        {
            ...
            if (!se)
            {
                pd->error("string expected for mangled name, not '%s'", e->toChars());
                goto Ldecl;
            }
            if (!se->len)
            {
    ...
    void funcDeclarationSemantic(FuncDeclaration *funcdecl)
    {
        ...
        if (fld && fld->treq)
        {
            ...
            if (treq->ty == Tdelegate)
                ...
                assert(0);
            funcdecl->linkage = ((TypeFunction *)treq->nextOf())->linkage;
        }
        else
        ...
        if (!funcdecl->type->deco)
        {
            ...
            TypeFunction *tf = (TypeFunction *)funcdecl->type;
            ...
            funcdecl->type = semantic(funcdecl->type, funcdecl->loc, sc);
            sc = sc->pop();
        }
        ...
        else
        {
            ...
            TypeFunction *tfo = (TypeFunction *)funcdecl->originalType;
            TypeFunction *tfx = (TypeFunction *)funcdecl->type;
            tfo->mod        = tfx->mod;
        ...
        */
        ...
        if (ClassDeclaration *cd = parent->isClassDeclaration())
        {
             ...
             */
            int vi = cd->baseClass ? funcdecl->findVtblIndex((Dsymbols*)&cd->baseClass->vtbl, (int)cd->baseClass->vtbl.dim)
                : -1;
                    ...
                    */
                    ...
                    else
                    {
                        ...
                        if (cd->cpp && Target::reverseCppOverloads)
                        {
                            ...
                            funcdecl->vtblIndex = (int)cd->vtbl.dim;
                ...
                default:
                    {
                        ...
                        if (!funcdecl->isOverride())
                        {
                            int vi2 = funcdecl->findVtblIndex(&cd->baseClass->vtbl, (int)cd->baseClass->vtbl.dim, false);
                            if (vi2 < 0)
        ...
        Linterfaces:
            for (size_t i = 0; i < cd->interfaces.length; i++)
            {
                ...
                vi = funcdecl->findVtblIndex((Dsymbols *)&b->sym->vtbl, (int)b->sym->vtbl.dim);
                switch (vi)
                {
                    case -1:
                        break;
        ...
        assert(funcdecl->type->ty == Tfunction);
        f = (TypeFunction *)funcdecl->type;
    ...
    void visit(CtorDeclaration *ctd)
    {
        ...
        TypeFunction *tf = (TypeFunction *)ctd->type;
    ...
    void visit(NewDeclaration *nd)
    {
        ...
        nd->type = semantic(nd->type, nd->loc, sc);
        assert(nd->type->ty == Tfunction);

        // Check that there is at least one argument of type size_t
        TypeFunction *tf = (TypeFunction *)nd->type;
    ...
    void visit(DeleteDeclaration *deld)
    {
        ...
        deld->type = semantic(deld->type, deld->loc, sc);
        assert(deld->type->ty == Tfunction);

        // Check that there is only one argument of type void*
        TypeFunction *tf = (TypeFunction *)deld->type;
        if (Parameter::dim(tf->parameters) != 1)
        {
    ...
    {
        //printf("StructDeclaration::semantic(this=%p, %s '%s', sizeok = %d)\n", sd, parent->toChars(), toChars(), sizeok);
        ...
        sd->type = semantic(sd->type, sd->loc, sc);
        ...
        if (!sd->determineFields())
        {
            ...
            {
                error(sd->loc, "circular or forward reference");
                errors = true;
                sd->type = Type::terror;
            }
        ...
        */
        ...
        {
            printf("this = %p %s\n", sd, sd->toChars());
            printf("type = %d sym = %p\n", sd->type->ty, ((TypeStruct *)sd->type)->sym);
        }
#endif
        assert(sd->type->ty != Tstruct || ((TypeStruct *)sd->type)->sym == sd);
    }
    ...
    static Type *resolveBase(ClassDeclaration *cd, Scope *sc, Scope *&scx, Type *type)
    {
        ...
        cd->_scope = scx;
        Type *t = semantic(type, cd->loc, sc);
        cd->_scope = NULL;
        return t;
    }
    ...
    void visit(ClassDeclaration *cldec)
    {
        ...
        //printf("+ClassDeclaration::semantic(%s), type = %p, sizeok = %d, this = %p\n", toChars(), cldec->type, sizeok, cldec);
        ...
        cldec->type = semantic(cldec->type, cldec->loc, sc);
        ...
        if (cldec->baseok < BASEOKdone)
        {
            ...
            if (!cldec->baseClass && cldec->ident != Id::Object && !cldec->cpp)
            {
                ...
                t = semantic(t, cldec->loc, sc)->toBasetype();
                if (t->ty == Terror)
                    badObjectDotD(cldec);
                assert(t->ty == Tclass);
            ...
            cldec->interfaces.length = cldec->baseclasses->dim - (cldec->baseClass ? 1 : 0);
    ...
    Lancestorsdone:
        ...
        if (cldec->baseok == BASEOKdone)
        {
            ...
            if (cldec->baseClass)
            {
                ...
                memcpy(cldec->vtbl.tdata(), cldec->baseClass->vtbl.tdata(), sizeof(void *) * cldec->vtbl.dim);
         ...
         */
        for (size_t i = 0; i < cldec->fields.dim; i++)
        {
        ...
        {
            FuncDeclaration *fd = resolveFuncCall(cldec->loc, sc2, cldec->baseClass->ctor, NULL, NULL, NULL, 1);
            if (fd && !fd->errors)
            {
        ...
        {
            const int isabstractsave = cldec->isabstract;
    ...
    void visit(InterfaceDeclaration *idec)
    {
        ...
        idec->type = semantic(idec->type, idec->loc, sc);
        ...
        if (idec->baseok < BASEOKdone)
        {
            ...
            idec->interfaces.length = idec->baseclasses->dim;
    ...
    Lancestorsdone:
        ...
        {
            printf("this = %p %s\n", idec, idec->toChars());
            printf("type = %d sym = %p\n", idec->type->ty, ((TypeClass *)idec->type)->sym);
        }
#endif
    ...
    void visit(VarDeclaration *dsym)
    {
        ...
        */
        ...
        else
        {
            ...
            dsym->inuse++;
            dsym->type = semantic(dsym->type, dsym->loc, sc2);
            dsym->inuse--;
            sc2->pop();
        }
        ...
        if (tb->ty == Ttuple)
        {
            ...
            if (nelems > 0 && ie)
            {
                ...
                Lexpand1:
                    ...
                    arg->type = semantic(arg->type, dsym->loc, sc);
        ...
        if (dsym->_init)
        {
            ...
            if (sc->func || sc->intypeof == 1)
            {
                    ...
                    !dsym->_init->isVoidInitializer())
                {
                    ...
                    if (!ei)
                    {
                        ...
                        if (!e)
                        {
                            ...
                            dsym->_init = semantic(dsym->_init, sc, dsym->type, INITnointerpret);
                            e = initializerToExpression(dsym->_init);
                            if (!e)
                            {
                ...
                else
                {
                    ...
                    dsym->_init = semantic(dsym->_init, sc, dsym->type, sc->intypeof == 1 ? INITnointerpret : INITinterpret);
                }
            }
                     ...
                     dsym->type->isConst() || dsym->type->isImmutable())
            {
                ...
                if (!inferred)
                {
                    ...
                    dsym->_init = semantic(dsym->_init, sc, dsym->type, INITinterpret);
                    dsym->inuse--;
                    if (global.errors > errors)
                    {
                        dsym->_init = new ErrorInitializer();
        ...
        */
        ...
        if (dsym->edtor)
        {
            if (sc->func && dsym->storage_class & (STCstatic | STCgshared))
                dsym->edtor = semantic(dsym->edtor, sc->_module->_scope);
            else
                dsym->edtor = semantic(dsym->edtor, sc);
    ...
    void visit(EnumDeclaration *ed)
    {
        ...
        ed->parent = sc->parent;
        ed->type = semantic(ed->type, ed->loc, sc);
        ...
        if (ed->memtype)
        {
            ed->memtype = semantic(ed->memtype, ed->loc, sc);
    ...
    void visit(EnumMember *em)
    {
        ...
        if (em->origType)
        {
            em->origType = semantic(em->origType, em->loc, sc);
            em->type = em->origType;
        ...
        else
        {
            ...
            if (emprev->semanticRun < PASSsemanticdone)    // if forward reference
                semantic(emprev, emprev->_scope);    // resolve it
            if (emprev->errors)
                goto Lerrors;
            ...
            if (e->op != TOKerror)  // avoid duplicate diagnostics
            {
                ...
                em->origValue = semantic(em->origValue, sc);
        ...
        if (imp->mod)
        {
            ...
            if (!imp->aliasId && !imp->names.dim) // neither a selective nor a renamed import
            {
                ...
                if (imp->packages)
                {
                    ...
                    for (size_t i = 1; i < imp->packages->dim; i++) // [b, c]
                    {
    ...
    {
        if (ns->semanticRun >= PASSsemantic)
            return;
        ...
        if (ns->members)
        {
            ...
            for (Scope *sce = sc; 1; sce = sce->enclosing)
            {
                ...
                if (sds)
                {
                    sds->importScope(ns, Prot(PROTpublic));
                    break;
                }
            }
    ...
    {
#if LOG
        printf("TemplateDeclaration::semantic(this = %p, id = '%s')\n", tempdecl, tempdecl->ident->toChars());
        printf("sc->stc = %llx\n", sc->stc);
        printf("sc->module = %s\n", sc->_module->toChars());
#endif
        ...
        for (size_t i = 0; i < tempdecl->parameters->dim; i++)
        {
            ...
            {
                tempdecl->error(tp->loc, "parameter '%s' multiply defined", tp->ident->toChars());
                tempdecl->errors = true;
            }
            if (!semantic(tp, paramscope, tempdecl->parameters))
            {
                tempdecl->errors = true;
            }
...
class Semantic2Visitor : public Visitor
{
    ...
    {
        dd->getMessage();
        visit((StorageClassDeclaration *)dd);
    }
...
#endif
            // Bugzilla 14166: Don't run CTFE for the temporary variables inside typeof
            vd->_init = semantic(vd->_init, sc, vd->type, sc->intypeof == 1 ? INITnointerpret : INITinterpret);
            vd->inuse--;
        }
...
void semantic(Dsymbol *dsym, Scope *sc)
{
    DsymbolSemanticVisitor v = DsymbolSemanticVisitor(sc);
    dsym->accept(&v);
}
...
void semantic2(Dsymbol *dsym, Scope *sc)
{
    Semantic2Visitor v = Semantic2Visitor(sc);
    dsym->accept(&v);
}

void templateInstanceSemantic(TemplateInstance *tempinst, Scope *sc, Expressions *fargs)
{
    //printf("[%s] TemplateInstance::semantic('%s', this=%p, gag = %d, sc = %p)\n", loc.toChars(), toChars(), tempinst, global.gag, sc);
#if 0
...
//    parent = scope->scopesym;
    ...
    else if (tempinst->tinst)
    {
        ...
        else if (sc->func)
        {
             ...
             */
            for (size_t i = 0; i < tempinst->tdtypes.dim; i++)
            {
...
void aliasSemantic(AliasDeclaration *ds, Scope *sc)
{
    ...
    if (!s) // it's a type alias
    {
        ...
        ds->type = semantic(ds->type, ds->loc, sc);
        ds->aliassym = NULL;
    }
