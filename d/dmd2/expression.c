...
SymOffExp::SymOffExp(Loc loc, Declaration *var, target_size_t offset, int hasOverloads)
...
{   Expression *e;
    ...
    switch (t1->ty)
    {
        ...
        {   TypeAArray *taa = (TypeAArray *)t1;
             ...
             */
            if (!arrayTypeCompatibleWithoutCasting(e2->loc, e2->type, taa->index) || e1->op == TOKnull)
            {
...
{   Expression *e;
    ...
    switch (t2b->ty)
    {
        case Taarray:
        {
            ...
            if (!arrayTypeCompatible(e1->loc, e1->type, ta->index) || e1->op == TOKnull)
#endif
            {
...
{   Expression *e;
    ...
    /* Disallow integer comparisons S>=U, S>U, S<=U and S<U
       ...
       The specs on integral comparisons is as follows:
       '''
       ...
       This implementation comes with the following deviations from the D spec:
       o Signed/unsigned comparison with positive integer literals will not trigger.

       o Whilst declared an error in the docs, will instead warn as it *will* break
         a lot of currently existing code, including that found in Phobos.
     */
